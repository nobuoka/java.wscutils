...
public class JsonParser {
	...
	static private class CodePointIterator {
		private String str;
		private int index;
		//private int numCP;
		private int length;
	...
	static private boolean isWS( int cp ) {
		return cp == 0x20 || cp == 0x09 || cp == 0x0A || cp == 0x0D;
	}
	
	static private Token getNextStringToken( CodePointIterator cpi ) {
	...
	static private Token getNextNumberToken( CodePointIterator cpi, int cp ) {
	...
	static private void checkNextLiteralToken( CodePointIterator cpi, int[] cps ) {
		for( int cp : cps ) {
	...
	static private Token getNextToken( CodePointIterator cpi ) {
	...
	static private JsonValue parseValue( CodePointIterator cpi, Token token ) {
		switch( token.type ) {
			case BEGIN_ARRAY:
				return parseArray( cpi, new JsonArray() );
			case BEGIN_OBJECT:
				return parseObject( cpi, new JsonObject() );
			case STRING:
	...
	static private JsonObject parseObject( CodePointIterator cpi, JsonObject jobject ) {
		Token token = getNextToken( cpi );
		...
		while( true ) {
			...
			// name separater
			token = getNextToken( cpi );
			if( token.type != Token.TokenType.NAME_SEPARATER )
				throw new RuntimeException( "unexpected token" );
			// value
			token = getNextToken( cpi );
			JsonValue value = parseValue( cpi, token );
	...
	static private JsonArray parseArray( CodePointIterator cpi, JsonArray jarray ) {
		Token token = getNextToken( cpi );
	...
	static public JsonValue parse( String jsonStr ) {
		// array or obj?
		CodePointIterator cpi = new CodePointIterator( jsonStr );
		Token token = getNextToken( cpi );
		if( token == null ) {
		...
		if( token.type == Token.TokenType.BEGIN_ARRAY ) {
			val = parseArray( cpi, new JsonArray() );
		} else if( token.type == Token.TokenType.BEGIN_OBJECT ) {
			val = parseObject( cpi, new JsonObject() );
		} else {
			throw new RuntimeException();
		}
		if( ( token = getNextToken( cpi ) ) != null ) {
			throw new RuntimeException();
		}
		return val;
	}
